...
"""
数据库对象对比工具 (V12 - ALTER 级别修复版)
对比 Oracle (源) 和 OceanBase (目标) 的 TABLE / VIEW 及相关对象，
并生成基于 ALTER/CREATE 的修补脚本。

在 V11 的基础上增加：
- 对“列不匹配”的 TABLE，不再尝试重建表，而是生成 ALTER TABLE 级别的修复脚本：
  - 对缺失列：ALTER TABLE ... ADD (col datatype [DEFAULT ...] [NOT NULL]);
  - 对多余列：生成注释掉的 ALTER TABLE ... DROP COLUMN col; 作为建议。
"""
...
def generate_fixup_scripts(
    ...
    基于校验结果生成 fix_up DDL 脚本：
      - TABLE / VIEW: 缺失对象 → CREATE DDL
      - INDEX / CONSTRAINT / TRIGGER / SEQUENCE: 缺失对象 → CREATE DDL
      - TABLE 列不匹配：生成 ALTER TABLE ADD / (建议) DROP COLUMN
    所有脚本仅写入文件，不在 OB 上执行。
    """
    if not master_list:
...
def print_final_report(
    ...
    print(f"           {Color.BOLD}数据库对象迁移校验报告 (V12){Color.ENDC}")
    ...
    print("  - fix_up/trigger     : 缺失触发器的 CREATE 脚本")
